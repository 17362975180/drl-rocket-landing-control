...
def parse_args():
    ...
    parser.add_argument("--output-dir", type=str, default="results/experiments/06_rl_algorithms")
...
def main():
    ...
    for i, name in enumerate(names):
        ...
        # Use paired evaluation seeds across PPO/SAC/TD3. Algorithm training
        # seeds remain independent and do not affect the fairness of testing.
        summary, rollouts = evaluate_algorithm(name, model_path, stats_path, args.n_episodes, args.seed)
        summary["algorithm"] = name
        summary["source_model"] = str(model_path)
        results[name] = summary
        trajectories[name] = {
            "comparison": rollouts[0] if rollouts else None,
            "comparison_seed": args.seed,
            "comparison_episode": 0,
