...
def evaluate_sb3_policy(spec: PolicySpec, scenarios, n_episodes, seed, save_trajectories):
    ...
    for idx, (scenario_name, options) in enumerate(scenarios.items()):
        ...
        if save_trajectories:
            representatives[scenario_name] = next((r for r in rollouts if r["success"]), rollouts[0] if rollouts else None)
...
def parse_args():
    ...
    parser.add_argument("--output-dir", type=str, default="results/reproducible/scenario_benchmark")
