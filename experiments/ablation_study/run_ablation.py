...
"""
消融实验 - 奖励函数消融研究
包含：训练、评估、鲁棒性测试、泛化性测试、轨迹对比图

奖励变体：
1. full - 完整奖励
2. no_fuel - 去掉燃料惩罚
3. no_smooth - 去掉动作平滑惩罚
4. no_safety - 去掉安全惩罚
5. no_success - 去掉成功奖励
6. basic - 只保留高度和速度误差
"""
...
def get_reward_breakdown(env, height, velocity, throttle, fuel, prev_throttle, done, info):
    """计算奖励分解"""
    rewards = {}

    # 高度误差奖励
    height_error = abs(height) / 50.0
    rewards['height'] = -height_error * 10.0

    # 速度误差奖励
    if height < 5.0:
        velocity_error = abs(velocity) / 10.0
        rewards['velocity'] = -velocity_error * 20.0
    else:
        velocity_error = abs(velocity - (-3.0)) / 10.0
        rewards['velocity'] = -velocity_error * 5.0

    # 燃料惩罚
    rewards['fuel'] = -0.1 * throttle

    # 动作平滑惩罚
    if prev_throttle is not None:
        rewards['smooth'] = -0.5 * abs(throttle - prev_throttle)
    else:
        rewards['smooth'] = 0.0

    # 安全惩罚
    if height < 10.0 and velocity < -5.0:
        rewards['safety'] = -5.0
    else:
        rewards['safety'] = 0.0
...
def train_ablation_variant(variant_name, total_timesteps=200000):
    """训练单个消融变体"""
    print(f"\n{'='*60}")
    print(f"训练消融变体: {variant_name}")
    print(f"{'='*60}")
    ...
    # 保存模型
    model_dir = Path(__file__).parent / "models" / variant_name
...
def evaluate_variant(variant_name, n_episodes=100):
    """评估单个消融变体"""
    print(f"\n评估消融变体: {variant_name}")

    # 尝试从本地目录加载，如果不存在则从final_report加载
    model_dir = Path(__file__).parent / "models" / variant_name
    if not (model_dir / "model.zip").exists():
        model_dir = project_root / "results" / "final_report" / "ablation" / variant_name
    ...
    for ep in range(n_episodes):
        ...
        while not done:
            ...
            episode_data['throttles'].append(throttle)
        ...
        # 计算油门变化率
        throttles = episode_data['throttles']
        if len(throttles) > 1:
...
def run_robustness_test(variant_name, n_episodes=30):
    """对消融变体进行鲁棒性测试"""
    print(f"\n鲁棒性测试: {variant_name}")
...
def run_generalization_test(variant_name, n_episodes=30):
    """对消融变体进行泛化性测试"""
    print(f"\n泛化性测试: {variant_name}")
...
def record_trajectory(variant_name, seed=42):
    """录制单个变体的轨迹"""
    # 尝试从本地目录加载，如果不存在则从final_report加载
    model_dir = Path(__file__).parent / "models" / variant_name
    if not (model_dir / "model.zip").exists():
...
def generate_trajectory_comparison():
    ...
    for i, variant in enumerate(variants):
        try:
            trajectory = record_trajectory(variant)
            time = np.arange(len(trajectory['heights'])) * 0.05

            axes[0, 0].plot(time, trajectory['heights'], color=colors[i], label=labels[i], linewidth=1.5, alpha=0.8)
            axes[0, 1].plot(time, trajectory['velocities'], color=colors[i], label=labels[i], linewidth=1.5, alpha=0.8)
            axes[1, 0].plot(time, trajectory['throttles'], color=colors[i], label=labels[i], linewidth=1.5, alpha=0.8)
            axes[1, 1].plot(time, trajectory['fuels'], color=colors[i], label=labels[i], linewidth=1.5, alpha=0.8)
        except Exception as e:
            print(f"  {variant} 轨迹录制失败: {e}")
    ...
    plt.suptitle('Ablation Study: Trajectory Comparison', fontsize=14, fontweight='bold')
    plt.tight_layout()
...
def generate_robustness_chart(robustness_results):
    """生成鲁棒性测试柱状图"""
    print("生成鲁棒性测试图...")

    scenarios = list(robustness_results.keys())
    variants = list(robustness_results[scenarios[0]].keys())
    ...
    for i, variant in enumerate(variants):
        success_rates = [robustness_results[s][variant]['success_rate'] for s in scenarios]
        ax.bar(x + i * width, success_rates, width, label=variant, color=colors[i % len(colors)], alpha=0.8)

    ax.set_xlabel('Robustness Scenario')
    ax.set_ylabel('Success Rate')
    ax.set_title('Ablation Study: Robustness Comparison')
...
def generate_generalization_chart(generalization_results):
    """生成泛化性测试图"""
    print("生成泛化性测试图...")

    scenarios = list(generalization_results.keys())
    variants = list(generalization_results[scenarios[0]].keys())
    n_scenarios = len(scenarios)
    n_variants = len(variants)
    ...
    for i, variant in enumerate(variants):
        success_rates = [generalization_results[s][variant]['success_rate'] for s in scenarios]
        ax.bar(x + i * width, success_rates, width, label=variant, color=colors[i % len(colors)], alpha=0.8)

    ax.set_xlabel('Generalization Scenario')
    ax.set_ylabel('Success Rate')
    ax.set_title('Ablation Study: Generalization Comparison')
    ax.set_xticks(x + width * (n_variants - 1) / 2)
    ax.set_xticklabels(scenarios, rotation=45, ha='right')
    ax.legend()
    ax.grid(True, alpha=0.3, axis='y')
...
def main():
    ...
    for variant in variants:
        try:
            stats = evaluate_variant(variant, n_episodes=100)
            eval_results[variant] = stats
            print(f"  {variant}: {stats['success_rate']*100:.1f}% success")
        except Exception as e:
            print(f"  {variant} 评估失败: {e}")
    ...
    for variant in variants:
        try:
            results = run_robustness_test(variant, n_episodes=30)
            robustness_results[variant] = results
        except Exception as e:
            print(f"  {variant} 鲁棒性测试失败: {e}")

    # 转换为场景-变体格式
    robustness_by_scenario = {}
    for variant, scenarios in robustness_results.items():
        for scenario, stats in scenarios.items():
            if scenario not in robustness_by_scenario:
                robustness_by_scenario[scenario] = {}
            robustness_by_scenario[scenario][variant] = stats
    ...
    for variant in variants:
        try:
            results = run_generalization_test(variant, n_episodes=30)
            generalization_results[variant] = results
        except Exception as e:
            print(f"  {variant} 泛化性测试失败: {e}")

    # 转换为场景-变体格式
    generalization_by_scenario = {}
    for variant, scenarios in generalization_results.items():
        for scenario, stats in scenarios.items():
            if scenario not in generalization_by_scenario:
                generalization_by_scenario[scenario] = {}
            generalization_by_scenario[scenario][variant] = stats
